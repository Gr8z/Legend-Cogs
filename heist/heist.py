...
class Heist:
    ...
    async def _clear_heist(self, ctx, user: discord.Member):
        ...
        await self.bot.say("```{} administratively cleared {}```".format(author.display_name, user.display_name))
    ...
    async def _bailout_heist(self, ctx, user: discord.Member=None):
        ...
        if settings["Players"][player.id]["Status"] != "Apprehended":
            return await self.bot.say("{} is not in jail.".format(player.display_name))
        ...
        if player.id == author.id:
            ...
            msg = ("You are about pay a {2} amount for {0} and it will cost you {1} credits. "
                   "Are you sure you wish to pay {1} for {0}?".format(player.display_name, cost, t_bail))
                ...
                for server in servers:
                    for target in self.system["Servers"][server]["Targets"]:
                        ...
                        if vault < vault_max:
                            ...
                            pass
                self.save_system()
                await asyncio.sleep(480)  # task runs every 480 seconds
    ...
    def game_outcomes(self, settings, players, target):
        ...
        for player in players:
            ...
            if chance <= success_rate:
                ...
                results.append(dropout_msg.format(player.display_name))
        self.save_system()
        return results
    ...
    def account_check(self, settings, author):
        if author.id not in settings["Players"]:
            criminal = {"Name": author.display_name, "Status": "Free", "Sentence": 0, "Time Served": 0,
    ...
    def reaper_hook(self, server, author, user):
        ...
        if settings["Players"][user.id]["Status"] == "Dead":
            msg = "Cast failed. {} is dead.".format(user.display_name)
            action = None
        else:
            self.run_death(settings, user)
            self.save_system()
            msg = ("{} casted :skull: `death` :skull: on {} and sent them "
                   "to the graveyard.".format(author.display_name, user.display_name))
            action = "True"
        return action, msg
